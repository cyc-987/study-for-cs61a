...
def swine_align(player_score, opponent_score):
    ...
    if player_score == 0 or opponent_score == 0:
        ...
        selected = min(player_score,opponent_score)
        if selected < 10:
            return False
        else:
            for i in range (10, selected+1):
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0 < goal and score1 < goal:
        if who == 0:
            pointsAdded = take_turn(strategy0(score0,score1),score1,dice)
            score0 += pointsAdded
            ...
            pointsAdded = take_turn(strategy1(score1,score0),score0,dice)
            score1 += pointsAdded
            say = say(score0,score1)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    if who == 1:
        def say(score0, score1):
            ...
            return announce_highest(1,score1,max(gain,running_high))
    else:
        def say(score0, score1):
            gain = 0
            if score0 != 0:
            ...
            return announce_highest(0,score0,max(gain,running_high))
    return say
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    # BEGIN PROBLEM 9
    "*** YOUR CODE HERE ***"
    evaluate = make_averaged(roll_dice,trials_count)
    max = 0
    maxindex = 1
    for i in range(1,11):
        result = evaluate(i,dice)
        if result > max:
            max = result
            maxindex = i
    return maxindex
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    addon = free_bacon(opponent_score)
    if extra_turn(score+addon, opponent_score): return 0
    elif addon >= cutoff: return 0
    else: return num_rolls
    # END PROBLEM 11
